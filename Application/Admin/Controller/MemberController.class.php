	...
	public function member_list_runadd(){
		if (!IS_AJAX){
			...
			$sl_data=array(
					...
					'member_list_address'=>I('member_list_address'),
 ...
 */
	...
	public function member_list_runedit(){
		if (!IS_AJAX){
			...
			$sl_data['member_list_address']=I('member_list_address');
 ...
 */
	public function member_list_del(){
		...
		$id=I('member_list_id');
        $path = "./Uploads/";
        $v = M('member_list')->where(array('member_list_id'=>$id))->find();
		...
		M('member_list')->where(array('member_list_id'=>$id))->delete();//转入回收站
